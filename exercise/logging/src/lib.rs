...
impl Frog {
    pub fn new() -> Self {
        ...
        debug!(target: "new()", "A new Frog is being created");
        Default::default()
    }
    pub fn hop(&mut self) {
        self.energy -= 1;
        info!(target: "hop()", "The Frog hopped, {} energy left", self.energy);
        // 3. Use info!() to log that a Frog hopped, and how much energy is left
        if self.energy == 0 {
            warn!(target: "hop()", "Frog will go to sleep since its energy is 0");
...
impl Default for Frog {
    fn default() -> Self {
        ...
        let default_frog = Frog {
            energy: 5,
            sleeping: false,
        };
        trace!(target: "default()", "Default value was generated: {:?}", default_frog);
        default_frog
    }
}
